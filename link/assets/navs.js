...
(() => {
    ...
    let rawLinkHTML = lines.forEach((link) => {
      if (isInABlock) {
        ...
        if (link.trim().indexOf(BLOCK_SPLIT) === 0) {
          // end of a block
          newHTMLLines.push(`<pre class='block' id='${blockId}' ondblclick='window.onCopyBlockToClipboard(this)'>${blockBuffer.trim()}</pre>`);
          isInABlock = false;
          blockBuffer = '';
      ...
      if (link.trim().indexOf(FAV_ICON_SPLIT) === 0) {
        ...
        if (linkType) {
          ...
          if (linkUrl.indexOf('javascript://') === 0) {
            ...
            // same tab link
            window.schemaCacheMap[newCacheId] = linkUrl;
            newHTMLLines.push(`<a class='link sameTabLink' data-target-id="${newCacheId}" data-section='${currentHeaderName}'>${linkText}</a>`);
          } else {
            // new_tab_link
            window.schemaCacheMap[newCacheId] = linkUrl;
            newHTMLLines.push(`<a class='link newTabLink' target="_blank" data-target-id="${newCacheId}" data-section='${currentHeaderName}'>${linkText}</a>`);
          }
        }
      }
    });
  ...
  window.setPageFavIcon = (pageFavIcon) => {
    ...
    const favIconEncoded =
      encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'><text x='0' y='14'>`) + pageFavIcon + encodeURIComponent(`</text></svg>`);
